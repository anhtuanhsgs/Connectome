...
def average_inference2 (vol_shape, shape, step, final_affs, weights_map, aug=True, name='tmp'):
	weights = build_w2 (shape)
	print 'Predicting.'
	...
	for rotn in range (4):
		for flipn in range (1, 5):
			...
			z0 = 0;
			while z0 < vol_shape[0]:
				x0 = 0;
				if z0+shape[0] >= vol_shape[0]:
					z0 = vol_shape[0] - shape[0]
				while x0 < vol_shape[1]:
					y0 = 0;
					if x0+shape[1] >= vol_shape[1]:
						x0 = vol_shape[1] - shape[1]
					while y0 < vol_shape[2]:
						if y0+shape[2] >= vol_shape[2]:
							y0 = vol_shape[2] - shape[2]
						print 'pos: ', z0, x0, y0, 'rot - flip:', rotn, flipn
						vol = volume[z0:z0+shape[0], x0:x0+shape[1], y0:y0+shape[2]]
						re_rotn = (4 - rotn) % 4
						re_flipn = flipn
						...
						final_affs[:, z0:z0+shape[0], x0:x0+shape[1], y0:y0+shape[2]] += np.multiply (pred_rev, weights)
			...
			final_affs *= 256
			final_affs = final_affs.astype (np.uint8)
			print ('saving')
			np.save ('aff_' + name + '_' + str (rotn) + '_' + str (flipn), final_affs)
	print 'Done'

def average_inference (vol_shape, shape, step, final_affs, weights_map, _rotn, _flipn, _revn, aug=True, name='tmp'):
	weights = build_w2 (shape)
	print 'Predicting.'
		...
		for x0 in range (0, vol_shape[1], step[1]):
			for y0 in range (0, vol_shape[2], step[2]):
				...
				if not aug:
					...
					for rotn in range (_rotn, _rotn + 1):
						for flipn in range (_flipn, _flipn + 1):
							# z prefine
							
							re_rotn = (4 - rotn) % 4
							re_flipn = flipn
							re_revn = revn
							aug_img = reverse (square_rotate (flip (vol, flipn), rotn), re_revn)
							aug_pred_affs  = predict_func (aug_img)[0]
							# aug_cnt += 1
							tmp = []
							for d in range (3):
							    tmp.append (flip (square_rotate (reverse (aug_pred_affs[d], re_revn), re_rotn), re_flipn))
							pred_rev = np.array (tmp)
							# z prefine
							if start_z == 1:
	...
	print 'Saving result.'
	np.save ('results/' + name + '_' + str (_rotn) + str (_flipn) + str (_revn) + '.npy', final_affs)
	print 'Done'
...
if __name__ == '__main__':
	...
	for rotn in range (4):
		for flipn in [1, 4]:
			for revn in [1, 2]:
				average_inference (vol_shape, shape, step, final_affs, weights_map, rotn, flipn, revn, name=model.name + '_' + dataset)
